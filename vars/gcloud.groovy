    ...
    withEnv(['GCLOUD_CONFIG=' + cwd + '/.gcloud', 'BOTO_CONFIG=' + cwd + '/.gcloud/boto.cfg']) {
      ...
      if (config["serviceAccountCredential"] != null) {
        withCredentials([file(credentialsId: config["serviceAccountCredential"], variable: 'GOOGLE_APPLICATION_CREDENTIALS')]) {
          sh 'cp $GOOGLE_APPLICATION_CREDENTIALS .gcloud/serviceaccount.json'
        }
      ...
      sh 'echo "$(pwd)/.gcloud/serviceaccount.json" | gsutil config -e -o "$(pwd)/.gcloud/boto.cfg"'
      sh 'gcloud auth activate-service-account --key-file="$(pwd)/.gcloud/serviceaccount.json"'
